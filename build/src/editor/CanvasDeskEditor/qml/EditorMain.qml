...
ApplicationWindow {
    ...
    function createPreviewObject(data) {
        var qml = ""
        if (data.type === "Button") {
            qml = 'import QtQuick; import QtQuick.Controls; import CanvasDesk; Button { text: "' + (data.text || "Button") + '"; x: ' + data.x + '; y: ' + data.y + '; onClicked: AppManager.launch("' + (data.exec || "") + '") }'
