...
bool WlrWindowManager::initialize() {
  ...
  if (!m_manager) {
    qWarning() << "[WLR] wlr_foreign_toplevel_management_v1 protocol not "
...
void WlrWindowManager::registryGlobal(void *data, struct wl_registry *registry,
  ...
  if (strcmp(interface, zwlr_foreign_toplevel_manager_v1_interface.name) == 0) {
    ...
    zwlr_foreign_toplevel_manager_v1_add_listener(self->m_manager,
                                                  &manager_listener, self);
    qInfo() << "[WLR] ✓ Bound to wlr_foreign_toplevel_manager_v1";
  }
}

void WlrWindowManager::registryGlobalRemove(void *data,
...
void WlrWindowManager::handleToplevel(
  ...
  zwlr_foreign_toplevel_handle_v1_add_listener(handle, &toplevel_listener,
                                               window);

  qInfo() << "[WLR] 🪟 New toplevel window detected! ID:" << window->id;
  qInfo() << "[WLR] Handle:" << handle;
}

void WlrWindowManager::handleFinished(
    void *data, struct zwlr_foreign_toplevel_manager_v1 *manager) {
  Q_UNUSED(manager)
  auto *self = static_cast<WlrWindowManager *>(data);
...
void WlrWindowManager::toplevelTitle(
  ...
  qInfo() << "[WLR] Window" << window->id << "updated title:" << window->title;
}

void WlrWindowManager::toplevelAppId(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *handle,
    const char *app_id) {
  Q_UNUSED(handle)
  auto *window = static_cast<WlrWindow *>(data);
  ...
  qInfo() << "[WLR] Window" << window->id << "updated app_id:" << window->appId;
}

void WlrWindowManager::toplevelState(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *handle,
...
void WlrWindowManager::toplevelDone(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *handle) {
  Q_UNUSED(handle)
  auto *window = static_cast<WlrWindow *>(data);
...
void WlrWindowManager::toplevelClosed(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *handle) {
  auto *window = static_cast<WlrWindow *>(data);
  auto *manager = qobject_cast<WlrWindowManager *>(window->parent());
...
void WlrWindowManager::toplevelParent(
    void *data, struct zwlr_foreign_toplevel_handle_v1 *handle,
    struct zwlr_foreign_toplevel_handle_v1 *parent) {
  Q_UNUSED(data)
  Q_UNUSED(handle)
  Q_UNUSED(parent)
  // Parent relationship event - not currently used, but handler required by
  // protocol
}
