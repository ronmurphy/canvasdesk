...
int main(int argc, char *argv[]) {
  ...
  parser.setApplicationDescription(
      "CanvasDesk - Customizable Wayland Desktop Environment");
  ...
  if (parser.isSet(installSessionOption)) {
    ...
    if (!QDir(sessionDir).exists()) {
      ...
      qWarning()
          << "You may need to create it first or check your X11 support.";
      return 1;
    }
    ...
    if (!QFile::exists(wrapperScript)) {
      ...
      qWarning()
          << "Please ensure canvasdesk-session exists in the project root.";
      return 1;
    }
    ...
    QString desktopEntry = "[Desktop Entry]\n"
                           "Name=CanvasDesk\n"
                           "Comment=Customizable X11 Desktop Environment with "
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      qInfo() << "CanvasDesk should now appear in your login manager (as an "
                 "X11 session).";
      qInfo() << "";
  ...
  if (parser.isSet(uninstallSessionOption)) {
    ...
    if (!QFile::exists(sessionFile)) {
      qInfo() << "Session file" << sessionFile
    ...
    if (QFile::remove(sessionFile)) {
      ...
      qWarning() << "You may need to run this with sudo:";
      qWarning() << "  sudo" << QCoreApplication::applicationFilePath()
                 << "--uninstallsession";
      return 1;
    }
  }
  ...
  // Register SystemMonitor
  qmlRegisterSingletonType<SystemMonitor>(
      "CanvasDesk", 1, 0, "SystemMonitor",
  ...
  const QUrl url(QUrl::fromLocalFile(QCoreApplication::applicationDirPath() +
                                     "/../src/editor/qml/" + qmlFile));
